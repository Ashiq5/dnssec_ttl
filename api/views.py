...
redis_pass = "20092010"
logger = logging.getLogger('dnssec_ttl')
...
async def _edit_zone_file(container_id, ttl, exp_id):
    ...
    try:
        ...
        cmd = "docker exec -i " + containers[container_id-1] + " sh -c 'cat > /etc/bind/zones/" + zone_file_name \
              + "' < " + path
        logger.debug("In edit zone " + path + " " + cmd)
        await _execute_bash(cmd)
        return True
    except Exception as e:
        traceback.print_exc()
    ...
    try:
        ...
        for j in stdout:
            if 'Zone fully signed:' in j:
                signed = True
        logger.debug("In sign " + ",".join(stdout))
        if not signed:
